...
- Instructions (e.g., _“Summarize the following article in one paragraph.”_)
- Examples of the task (e.g., input–output pairs)
- Constraints on style or format (e.g., _“Answer in JSON.”_)
...
- Uses the model as a general conditional distribution  
  \[p\_\theta(\text{output} \mid \text{prompt}).\]
...
A well-known illustration of zero-shot behavior comes from summarization with the prompt “TL;DR:”. Even without any fine-tuning or task-specific examples, GPT-2 can produce a reasonable one-sentence summary simply by appending “TL;DR:” to the end of an article. As shown in quantitative benchmarks on the CNN/DailyMail dataset, this simple zero-shot prompt achieves ROUGE scores (e.g., R-1 of 29.34) that are competitive with baselines like random selection (R-1 28.78) and approach supervised sequence-to-sequence models (R-1 31.33). This demonstrates that the model's pre-training on diverse web text, where “TL;DR:” conventionally introduces a summary, effectively allows it to perform specific downstream tasks without explicit parameter optimization.
...
We show the model a few examples \((x*i, y_i)\) in the prompt.  
Then we give it a new \(x*{\text{test}}\) and ask it to generate \(y\_{\text{test}}\).

All adaptation happens implicitly through the **forward pass** on the prompt.

This is sometimes described as _“learning via inference rather than via gradient updates.”_
...
- Instead of writing a textual prefix like _“You are a helpful assistant.”_, we insert a sequence of trainable embedding vectors before the input tokens.
...
- **Prefix-tuning as continuous prompts:** Instead of learning all model weights, the paper learns a short _continuous prefix_ of activations that every token can attend to, acting like “virtual tokens” at every layer of the Transformer while the base LM stays frozen. This is more expressive than just learning text prompts.
...
- **Low-data and extrapolation benefits:** In very small-data regimes and when evaluating on **unseen topics/domains** (e.g., unseen WebNLG categories or news-to-sports summarization), prefix-tuning often _outperforms_ full fine-tuning, suggesting that freezing the base LM can actually help generalization.

- **Design choices for continuous prompts:** The paper empirically studies several design choices: optimal prefix length (too long overfits), why tuning _all layers_ of the prefix beats tuning only the embedding layer (closer to discrete prompt tuning), and how initializing prefixes with activations of real words like “summarize” or “table-to-text” works better than random initialization.
